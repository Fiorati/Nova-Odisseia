...
export async function createPendingNavigatorInvite(actorUserId: number, input: { name: string; email: string }) {
  ...
  await recordSecurityAuditEvent({ actorUserId, eventType: existing ? "navigator_invite_resent" : "navigator_invite_created", targetType: "user", targetId: userId, scope: email });
