...
const campaignInputSchema = z.object({
  scope: z.enum(["polo", "distrital"]),
  regional: z.string().trim().max(120).optional(),
  district: z.string().trim().max(120).optional(),
  polo: z.string().trim().max(120).optional(),
  title: z.string().trim().min(2).max(160),
  objective: z.string().trim().max(3000).optional(),
  status: z.enum(["rascunho", "ativa", "concluida", "arquivada"]),
  startAt: z.date().nullable().optional(),
  endAt: z.date().nullable().optional(),
  targetParticipants: z.number().int().min(0).max(10000),
  targetParticipationRate: z.number().min(0).max(100),
  targetPsvs: z.number().int().min(0).max(10000),
  targetTpv: z.number().min(0),
  targetNewClients: z.number().int().min(0).max(100000),
  actualParticipants: z.number().int().min(0).max(10000).optional(),
  actualPsvs: z.number().int().min(0).max(10000).optional(),
  actualTpv: z.number().min(0).optional(),
  actualNewClients: z.number().int().min(0).max(100000).optional(),
});
...
export const appRouter = router({
  ...
  auth: router({
    ...
    viewAsStatus: publicProcedure.query(({ ctx }) =>
      ctx.viewer && ctx.user
        ? { viewing: true as const, viewer: { id: ctx.viewer.id, email: ctx.viewer.email }, target: { id: ctx.user.id, email: ctx.user.email, name: ctx.user.name } }
        : { viewing: false as const, viewer: null, target: null }
    ),
      ...
      .mutation(async ({ input }) => {
        ...
        if (
          ...
          (exists.loginMethod !== "team-invite" ||
            (await getCredentialsForUser(exists.id)))
        )
          throw new Error("Já existe uma conta com este e-mail.");
        const verification = await createVerificationCode();
        await saveEmailVerificationChallenge({
          email,
          name: input.name,
          leadershipRole: input.leadershipRole,
          codeHash: verification.passwordHash,
          codeSalt: verification.passwordSalt,
    ...
    verifyRegistrationCode: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (challenge.attempts >= 5)
          throw new Error(
            "Você excedeu as tentativas. Solicite um novo código."
          );
        if (
          ...
          await updateEmailVerificationChallenge(email, {
        ...
        await updateEmailVerificationChallenge(email, {
          verificationTokenHash: token.passwordHash,
          verificationTokenSalt: token.passwordSalt,
          verifiedAt: new Date(),
        });
    ...
    completeRegistration: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          ...
          throw new Error(
            "Confirmação inválida ou expirada. Solicite um novo código."
          );
        if (
          !(await verifyPassword(
            input.verificationToken,
            challenge.verificationTokenSalt,
            challenge.verificationTokenHash
          ))
        )
          throw new Error("Confirmação inválida. Solicite um novo código.");
        const exists = await getUserByEmail(email);
        const credentials = await createPassword(input.password);
        const user = exists
          ? await activatePendingEmailAccount(exists.id, {
              name: challenge.name,
    ...
    requestPasswordReset: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const verification = await createVerificationCode();
        await savePasswordResetChallenge({
          userId: user.id,
          codeHash: verification.passwordHash,
          codeSalt: verification.passwordSalt,
          expiresAt: new Date(Date.now() + 15 * 60 * 1000),
    ...
    verifyPasswordReset: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (
          ...
          await updatePasswordResetChallenge(user.id, {
            attempts: challenge.attempts + 1,
          });
          throw new Error("Código inválido, expirado ou indisponível.");
        }
        const token = await createVerificationToken();
        await updatePasswordResetChallenge(user.id, {
          resetTokenHash: token.passwordHash,
          resetTokenSalt: token.passwordSalt,
          verifiedAt: new Date(),
        });
    ...
    resetPassword: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          ...
          throw new Error("Redefinição inválida ou expirada.");
        if (
          !(await verifyPassword(
            input.resetToken,
            challenge.resetTokenSalt,
            challenge.resetTokenHash
          ))
        )
          throw new Error("Redefinição inválida ou expirada.");
        const credentials = await createPassword(input.password);
        await replaceEmailPassword(
          user.id,
  ...
  agent: router({
    ...
    currentGoal: protectedProcedure
      .input(z.object({ monthKey: z.string().regex(/^\d{4}-\d{2}$/) }))
      .query(({ ctx, input }) => getPrivateGoal(ctx.user.id, input.monthKey)),
    leadership: protectedProcedure
      .input(z.object({ role: z.enum(["none", "polo", "distrital"]) }))
      .mutation(async ({ ctx, input }) => {
  ...
  team: router({
    ...
    updateMember: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        updateTeamMemberOrganization(ctx.user.id, input.targetUserId, input)
      ),
    ...
    savePromise: protectedProcedure
      .input(
        z.object({
          targetUserId: z.number().int().positive().optional(),
          promiseDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
          salesTasks: z.number().int().min(0).max(100000).optional(),
          proposals: z.number().int().min(0).max(100000),
          newClients: z.number().int().min(0).max(100000),
          newClientsTpv: z.number().min(0).max(1_000_000_000),
          closedTpv: z.number().min(0).max(1_000_000_000).optional(),
    ...
    schedules: protectedProcedure
      .input(z.object({ targetUserId: z.number().int().positive().optional() }))
      .query(({ ctx, input }) =>
        listTeamSchedules(ctx.user.id, input.targetUserId)
      ),
    saveSchedule: protectedProcedure
      .input(
        z.object({
          id: z.number().int().positive().optional(),
          scopeType: z.enum(["user", "polo", "district", "regional"]),
          scopeId: z.number().int().positive().nullable().optional(),
          dupla: z.string().max(120).nullable().optional(),
      ...
      .mutation(({ ctx, input }) => saveTeamSchedule(ctx.user.id, input)),
    deactivateSchedule: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) =>
        deactivateTeamSchedule(ctx.user.id, input.id)
      ),
  ...
  monthlyCard: router({
    ...
    save: protectedProcedure
      .input(monthlyFinalCardInputSchema)
      .mutation(async ({ ctx, input }) => {
  ...
  portfolio: router({
    ...
    assignRoute: protectedProcedure
      ...
      .mutation(({ ctx, input }) => assignRouteOwner(ctx.user.id, input)),
    getForMyRoute: protectedProcedure
      .input(
        z.object({
    ...
    importStoneLeadList: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(({ ctx, input }) => importStoneLeadList(ctx.user.id, input)),
    googleSheet: protectedProcedure
      .input(
        z.object({
          url: z.string().url().max(2000),
          scope: z.enum(["polo", "district"]),
        })
      )
      .mutation(({ ctx, input }) =>
        getSharedGoogleSheetFile(ctx.user.id, input)
      ),
    ...
    importDistrict: protectedProcedure
      .input(
        z.object({
          portfolioType: z.enum(["base", "route"]),
          fileName: z.string().trim().min(1).max(255),
      ...
      .mutation(({ ctx, input }) =>
        importDistrictPortfolio(ctx.user.id, input)
      ),
  }),
  simulation: router({
    save: protectedProcedure
  ...
  campaigns: router({
    list: protectedProcedure.query(({ ctx }) =>
      listEngagementCampaigns(ctx.user.id)
    ),
    create: protectedProcedure
      .input(campaignInputSchema)
      .mutation(({ ctx, input }) =>
        createEngagementCampaign(ctx.user.id, input)
      ),
    update: protectedProcedure
      .input(campaignInputSchema.extend({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) => {
        const { id, ...campaign } = input;
        return updateEngagementCampaign(ctx.user.id, id, campaign);
      }),
  }),
  bestPractices: router({
    ...
    create: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        createBestPracticePost(ctx.user.id, ctx.user.name ?? "Agente", input)
      ),
    remove: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) =>
        removeBestPracticePost(ctx.user.id, ctx.user.role === "admin", input.id)
      ),
  }),
  meetings: router({
    list: protectedProcedure.query(() => listMeetingLeads()),
    save: protectedProcedure.input(z.object({
      id: z.number().int().positive().optional(),
      polo: z.string().max(120).optional(),
      cnpj: z.string().max(40),
      tradeName: z.string().trim().min(2).max(160),
      tpv: z.number().min(0),
      segment: z.string().max(160),
      route: z.string().max(120),
      decisionMaker: z.string().max(160),
      contact: z.string().max(160),
      notes: z.string().max(4000).optional(),
      status: z.enum(["novo", "contato", "agendada", "realizada", "cancelada"]),
  ...
  psv: router({
    ...
    saveDemand: protectedProcedure
      ...
      .mutation(({ ctx, input }) => savePsvDemand(ctx.user.id, input)),
    removeDemand: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) => removePsvDemand(ctx.user.id, input.id)),
    history: protectedProcedure.query(({ ctx }) =>
      getPrivatePlanHistory(ctx.user.id)
    ),
    pipeline: protectedProcedure.query(({ ctx }) =>
      getPrivatePipelineLeads(ctx.user.id)
    ),
    weeklyRitual: protectedProcedure
      .input(z.object({ weekOf: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) }))
      .query(({ ctx, input }) => getPsvWeeklyRitual(ctx.user.id, input.weekOf)),
    saveLead: protectedProcedure
      .input(
        z.object({
          id: z.number().int().positive().optional(),
          clientName: z.string().trim().min(2).max(160),
          temperature: z.enum(["quente", "frio"]),
          segmentId: z.string().max(96),
          segmentLabel: z.string().max(160),
          mcc: z.string().max(8),
          cnae: z.string().max(24),
          projectedTpv: z.number().min(0),
          nextContactAt: z.date().nullable().optional(),
          stage: z.enum(["mapeado", "qualificando", "planejado", "negociacao"]),
        })
      )
      .mutation(async ({ ctx, input }) => {
    ...
    saveWeeklyRitual: protectedProcedure
      .input(
        z.object({
          weekOf: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
          dailyResult: z.string().max(3000).optional(),
      ...
      .mutation(({ ctx, input }) => savePsvWeeklyRitual(ctx.user.id, input)),
    save: protectedProcedure
      .input(
        z.object({
          weekOf: z.string().min(8).max(10),
          targetVariable: z.number().min(0),
          currentVariable: z.number().min(0),
          plannedMultiplier: z.number().min(0.1).max(3),
          weeksRemaining: z.number().int().min(1).max(8),
          recommendedTpv: z.number().min(0),
          recommendedClients30: z.number().int().min(0),
          recommendedClients50: z.number().int().min(0),
          recommendedClients100: z.number().int().min(0),
    ...
    save: protectedProcedure
      .input(
        z.object({
          ...
          salesTasks: z.number().int().min(0),
          proposals: z.number().int().min(0),
          closedClients: z.number().int().min(0),
          closedTpv: z.number().min(0),
          goalTpv: z.number().min(0),
          variableValue: z.number().min(0),
          rootCause: z.string().trim().max(3000).optional(),
          actionPlan: z.string().trim().max(3000).optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const pointsAwarded = await saveRmrRecord(ctx.user.id, {
          ...input,
          globalKpi,
          pointsAwarded: 0,
        });
  ...
  prospection: router({
    list: protectedProcedure.query(({ ctx }) =>
      listProspectionDossiers(ctx.user.id)
    ),
    recommendations: protectedProcedure.query(({ ctx }) =>
      getProspectionRecommendations(ctx.user.id)
    ),
    create: protectedProcedure
      .input(
        z.object({
          ...
          routeEntryId: z.number().int().positive().optional(),
          hypotheses: z.string().max(3000).optional(),
          agentNotes: z.string().max(3000).optional(),
        })
      )
      .mutation(({ ctx, input }) =>
        createProspectionDossier(ctx.user.id, input)
      ),
    update: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(({ ctx, input }) =>
        updateProspectionDossier(ctx.user.id, input)
      ),
    researchCnpj: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(({ ctx, input }) =>
        researchProspectionCnpj(ctx.user.id, input)
      ),
  }),
  nordic: router({
    get: protectedProcedure
      ...
      .query(({ ctx, input }) =>
        getNordicStrategy(ctx.user.id, input.monthKey, input.semesterKey)
      ),
    savePlan: protectedProcedure
      .input(
        z.object({
          monthKey: z.string().regex(/^\d{4}-\d{2}$/),
          targetSalesTasks: z.number().int().min(0),
          targetProposals: z.number().int().min(0),
          targetClients7To15: z.number().int().min(0),
          targetClients15To30: z.number().int().min(0),
          targetClients30To50: z.number().int().min(0),
          targetClients50To100: z.number().int().min(0),
          targetClients100To200: z.number().int().min(0),
          targetClients200Plus: z.number().int().min(0),
          targetTpv: z.number().min(0),
          averageRv7To15: z.number().min(0),
          averageRv15To30: z.number().min(0),
          averageRv30To50: z.number().min(0),
          averageRv50To100: z.number().min(0),
          averageRv100Plus: z.number().min(0),
          actualSalesTasks: z.number().int().min(0),
          actualProposals: z.number().int().min(0),
          actualClients7To15: z.number().int().min(0),
          actualClients15To30: z.number().int().min(0),
          actualClients30To50: z.number().int().min(0),
          actualClients50To100: z.number().int().min(0),
          actualClients100To200: z.number().int().min(0),
          actualClients200Plus: z.number().int().min(0),
          actualTpv: z.number().min(0),
        })
      )
      .mutation(async ({ ctx, input }) => {
    ...
    saveActivation: protectedProcedure
      .input(
        z.object({
          id: z.number().int().positive().optional(),
          clientName: z.string().trim().min(2).max(160),
          stoneCode: z.string().max(48),
          mcc: z.string().max(8),
          cnae: z.string().max(24),
          segment: z.string().max(160),
          realTpv: z.number().min(0),
          projectedTpv: z.number().min(0),
          productsReady: z.boolean(),
          d15Complete: z.boolean(),
          d30Complete: z.boolean(),
          estimatedVariable: z.number().min(0),
          status: z.enum([
            "planejado",
            "ativacao",
            "acompanhamento",
    ...
    removeActivation: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => {
        await removeNordicActivationPlan(ctx.user.id, input.id);
        return { success: true };
      }),
    saveMicroRoute: protectedProcedure
      .input(
        z.object({
          id: z.number().int().positive().optional(),
          ...
          visitAt: z.date(),
          clientName: z.string().trim().min(2).max(160),
          pipelineLeadId: z.number().int().positive().nullable().optional(),
          priority: z.enum(["normal", "quente", "cem_mais"]),
          objective: z.string().trim().min(2).max(240),
          status: z.enum(["planejada", "concluida", "remarcada"]),
          notes: z.string().max(4000).optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
    ...
    removeMicroRoute: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => {
  ...
  ranking: router({
    ...
    tioPatinhas: protectedProcedure
      .input(z.object({ monthKey: z.string().regex(/^\d{4}-\d{2}$/) }))
      .query(({ input }) => getTioPatinhasLeaderboard(input.monthKey)),
    trophies: protectedProcedure
      .input(z.object({ monthKey: z.string().regex(/^\d{4}-\d{2}$/) }))
      .query(({ input }) => getMonthlyTrophies(input.monthKey)),
  }),
  spartacus: router({
    list: protectedProcedure.query(({ ctx }) => listSpartacusPdis(ctx.user.id)),
    progress: protectedProcedure.query(({ ctx }) =>
      getSpartacusProgressSummary(ctx.user.id)
    ),
    updateProgress: protectedProcedure
      .input(
        z.object({
          pdiId: z.number().int().positive(),
          skillId: z.string().trim().min(1).max(80),
          progressPercent: z.number().int().min(0).max(100),
        })
      )
      .mutation(({ ctx, input }) =>
        updateSpartacusSkillProgress(ctx.user.id, input)
      ),
    create: protectedProcedure
      .input(
        z.object({
  ...
  oraculo: router({
    ...
    disc: protectedProcedure.query(({ ctx }) => getDisc(ctx.user.id)),
    generateDisc: protectedProcedure.input(discInputSchema).mutation(({ ctx, input }) => generateDisc(ctx.user, input)),
    ...
    updateProfile: protectedProcedure
      .input(
        z.object({
          ...
          displayName: z.string().trim().min(2).max(120),
          leadershipRole: z.enum(["none", "polo", "distrital"]),
          regional: z.string().trim().max(120),
          district: z.string().trim().max(120),
          polo: z.string().trim().max(120),
          route: z.string().trim().max(120),
          targetVariable: z.number().min(0).max(100_000_000),
          defaultGoalTpv: z.number().min(0).max(1_000_000_000),
          defaultGoalNewClients: z.number().int().min(0).max(1_000_000),
        })
      )
      .mutation(({ ctx, input }) => updateAdminProfile(ctx.user.id, input)),
  }),
  leadership: router({
    districtLeaders: protectedProcedure.query(({ ctx }) =>
      getDistrictLeaderOverview(ctx.user.id)
    ),
    salesPipeline: protectedProcedure
      .input(
        z.object({
          monthKey: z
            .string()
      ...
      .query(({ ctx, input }) => getSalesPipelineForLeader(ctx.user.id, input)),
    overview: protectedProcedure
      .input(
        z.object({
          query: z.string().max(120).optional(),
          stage: z
            .enum(["all", "mapeado", "qualificando", "planejado", "negociacao"])
            .optional(),
          temperature: z.enum(["all", "quente", "frio"]).optional(),
          regional: z.string().max(120).optional(),
          district: z.string().max(120).optional(),
          polo: z.string().max(120).optional(),
        })
      )
      .query(({ ctx, input }) => getLeaderOverview(ctx.user.id, input)),
  }),
});
