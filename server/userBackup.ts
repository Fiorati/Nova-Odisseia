...
export async function buildUserBackup(userId: number) {
  ...
  const sections: BackupSection[] = [
    ...
    { title: "PSV - ritual semanal", rows: await db.select({
      id: psvWeeklyRituals.id,
      userId: psvWeeklyRituals.userId,
      weekOf: psvWeeklyRituals.weekOf,
      dailyResult: psvWeeklyRituals.dailyResult,
      dailyPlan: psvWeeklyRituals.dailyPlan,
      weeklyRoute: psvWeeklyRituals.weeklyRoute,
      preparedLeadIdsJson: psvWeeklyRituals.preparedLeadIdsJson,
      updatedAt: psvWeeklyRituals.updatedAt,
    }).from(psvWeeklyRituals).where(eq(psvWeeklyRituals.userId, userId)) },
    ...
    { title: "Notícias publicadas", rows: await db.select({
      id: newsArticles.id,
      authorUserId: newsArticles.authorUserId,
    ...
    }).from(newsArticles).where(eq(newsArticles.authorUserId, userId)) },
