...
function emptyLead(): Omit<MeetingLead, "id" | "polo"> {
  return {
    cnpj: "",
    tradeName: "",
    tpv: 0,
...
export default function MeetingSchedulingPanel() {
  ...
  const save = trpc.meetings.save.useMutation({
    onSuccess: async () => {
  ...
  const savePeriod = trpc.meetings.savePeriod.useMutation({
    onSuccess: async () => {
      await period.refetch();
      toast.success("Período de agendamento salvo.");
    },
    onError: error => toast.error(error.message),
  });
  useEffect(() => {
    if (period.data)
      setMetrics({
        callsMade: period.data.callsMade,
        callsAnswered: period.data.callsAnswered,
        meetingsBooked: period.data.meetingsBooked,
        clientsCredited: period.data.clientsCredited,
      });
  }, [period.data]);
  const routes = useMemo(
    () =>
      Array.from(
        new Set((team.data ?? []).flatMap(item => item.formalRoutes))
      ).sort(),
    [team.data]
  );
  const polos = Array.from(
    new Set((meetings.data ?? []).map(item => item.polo).filter(Boolean))
  );
  const submit = () => save.mutate({ ...draft, id: editing ?? undefined });
  const edit = (row: MeetingLead) => {
    setEditing(row.id);
    setDraft({
      cnpj: row.cnpj,
        ...
        <div className="mt-2 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <h2 className="text-3xl font-semibold tracking-[-.055em]">
              Encontros que movimentam o polo.
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-100/75">
              SDR, Jovem Aprendiz, agentes, líderes e distritais registram
              oportunidades e acompanham o período de agendamento por Polo.
            </p>
          </div>
          <UsersRound className="text-lime-200" size={28} />
        </div>
      </section>
      <section className="rounded-xl border border-emerald-100 bg-white p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 text-xl font-semibold">Registro rápido</h3>
          </div>
          <Button
            variant="outline"
            onClick={() => {
          ...
          <label className="grid gap-1 text-xs">
            ...
            <Input
              ...
              onChange={event =>
                setDraft(current => ({
                  ...current,
                  tpv: Number(event.target.value) || 0,
                }))
              }
            />
          </label>
          ...
          <label className="grid gap-1 text-xs">
            ...
            <Input
              ...
              onChange={event =>
                setDraft(current => ({
                  ...current,
                  decisionMaker: event.target.value,
                }))
              }
            />
          </label>
        ...
        <Button
          ...
          disabled={save.isPending || !draft.tradeName.trim()}
          onClick={submit}
        >
          <Save size={16} /> {editing ? "Atualizar reunião" : "Salvar reunião"}
        </Button>
      </section>
      <section className="rounded-xl border border-emerald-100 bg-white p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 text-xl font-semibold">Ritmo do polo</h3>
          </div>
          <label className="grid gap-1 text-xs">
            Competência
            <Input
              ...
              value={periodKey}
              onChange={event =>
                setPeriodKey(event.target.value || currentPeriod())
              }
            />
          </label>
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-4">
          {(
            [
              ["callsMade", "Ligações realizadas"],
              ["callsAnswered", "Ligações atendidas"],
              ["meetingsBooked", "Reuniões agendadas"],
              ["clientsCredited", "Clientes credenciados"],
            ] as const
          ).map(([key, label]) => (
        ...
        <Button
          ...
          onClick={() => savePeriod.mutate({ periodKey, ...metrics })}
        >
          <CalendarDays size={16} /> Salvar período
        </Button>
      </section>
      <section className="rounded-xl border border-emerald-100 bg-white p-5">
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full bg-lime-100 px-3 py-1 text-xs font-semibold text-emerald-900">
            {meetings.data?.length ?? 0} reuniões no escopo
          </span>
          {polos.map(polo => (
            <span
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[980px] text-left text-xs">
            <thead className="border-b border-emerald-100 font-mono text-[10px] uppercase tracking-[.08em] text-emerald-700/65">
              <tr>
                <th className="pb-3">CNPJ / STONECODE</th>
                <th className="pb-3">Nome fantasia</th>
                <th className="pb-3">TPV</th>
                <th className="pb-3">Segmento</th>
                <th className="pb-3">Rota</th>
                <th className="pb-3">Decisor</th>
            ...
            <tbody>
              {meetings.data?.map(row => (
                <tr className="border-b border-emerald-50" key={row.id}>
                  <td className="py-3">{row.cnpj || "—"}</td>
                  <td className="py-3 font-semibold">
                    {row.tradeName}
                    <small className="block text-emerald-700/55">
                      {row.polo || "Polo não informado"}
                    </small>
                  </td>
                  <td className="py-3">{row.tpv ? `R$ ${Number(row.tpv).toLocaleString("pt-BR")}` : "—"}</td>
                  <td className="py-3">{row.segment || "—"}</td>
                  <td className="py-3">{row.route || "—"}</td>
                  <td className="py-3">{row.decisionMaker || "—"}</td>
              ...
              {!meetings.data?.length && (
                <tr>
                  <td
                    ...
                    colSpan={9}
                  >
                    Nenhum registro neste escopo. Comece pelo formulário acima.
                  </td>
                </tr>
              )}
            </tbody>
