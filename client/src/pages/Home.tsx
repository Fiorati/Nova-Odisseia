      ...
      <div className="relative flex items-center gap-4">
        ...
        >
          ...
          <circle
            ...
            style={{
              filter: "drop-shadow(0 0 6px rgba(0,212,126,.85))",
        ...
        <div className="min-w-0">
          <p className="font-mono text-[10px] font-semibold tracking-[.12em] text-[#8fe6bd]">
            {label}
          </p>
...
function NovaOdisseiaLaunchBanner() {
  return (
    <section className="relative overflow-hidden rounded-2xl border border-[#00d47e]/30 bg-[#002b1d] p-6 text-white shadow-[0_20px_60px_rgba(0,43,29,.18)] md:p-8">
      <div className="absolute -right-16 -top-20 h-64 w-64 rounded-full border border-[#d4af37]/35 shadow-[0_0_0_24px_rgba(0,212,126,.08),0_0_0_52px_rgba(212,175,55,.06)]" />
      <div className="relative flex flex-col items-start gap-5 md:flex-row md:items-center">
        <ItakaEmblem size={82} />
        <div>
          <p className="font-mono text-[10px] font-semibold tracking-[.18em] text-[#8fe6bd]">
            NOVA ODISSEIA 2.0
          </p>
          <h2 className="mt-2 text-3xl font-semibold tracking-[-.055em] md:text-4xl">
            A jornada do agente Stone, da meta à Ítaka.
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-emerald-100/75">
            Uma plataforma de inteligência, gestão e desenvolvimento para
            planejar, executar, medir, corrigir e evoluir.
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-[#d4af37]">
...
function ProfilePanel({
  ...
  const [form, setForm] = useState(() => ({
    ...
    targetVariable: profile?.targetVariable ?? 0,
    defaultGoalTpv: profile?.defaultGoalTpv ?? 300000,
    defaultGoalNewClients: profile?.defaultGoalNewClients ?? 0,
    profileVisibleInRanking: profile?.profileVisibleInRanking ?? true,
    leadershipRole: profile?.leadershipRole ?? ("none" as LeadershipRole),
  ...
  useEffect(() => {
    if (profile)
      setForm({
        ...
        defaultGoalTpv: profile.defaultGoalTpv,
        defaultGoalNewClients: profile.defaultGoalNewClients,
        profileVisibleInRanking: profile.profileVisibleInRanking,
  ...
  const save = trpc.agent.profile.useMutation({
    onSuccess: async () => {
  ...
  const functionLabel =
    form.leadershipRole === "polo"
      ? "Dono de Polo"
      : form.leadershipRole === "distrital"
        ? "Distrital"
        : "Agente";
  return (
    <div className="max-w-4xl space-y-6">
      <section>
        ...
        <h2 className="mt-2 text-3xl font-semibold tracking-[-.055em]">
          Seu perfil e escopo operacional.
        </h2>
        <p className="mt-2 text-sm text-emerald-800/65">
          Funções de liderança e seus escopos são provisionados por
          administração autorizada para proteger informações de outros agentes.
        </p>
      </section>
      <section className="rounded-xl border border-emerald-100 bg-white p-6">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="grid gap-1 text-xs">
            <span>Função</span>
            <p className="rounded-md border border-emerald-100 bg-[#f5f8f2] px-3 py-2 text-sm font-semibold text-emerald-900">
              {functionLabel}
            </p>
          </div>
          {!isLeader && (
            <label className="grid gap-1 text-xs">
              ...
              <Input
                ...
                onChange={e =>
                  setForm(current => ({
                    ...current,
                    targetVariable: Number(e.target.value) || 0,
                  }))
                }
              />
            </label>
          ...
          <label className="grid gap-1 text-xs">
            {isLeader ? "Meta coletiva padrão de TPV" : "Meta padrão de TPV"}
            <Input
              type="number"
              value={form.defaultGoalTpv}
              onChange={e =>
                setForm(current => ({
                  ...current,
                  defaultGoalTpv: Number(e.target.value) || 0,
                }))
              }
            />
          </label>
          {isLeader && (
            <label className="grid gap-1 text-xs">
              Meta coletiva de novos clientes
              <Input
                type="number"
                value={form.defaultGoalNewClients}
                onChange={e =>
                  setForm(current => ({
                    ...current,
                    defaultGoalNewClients: Number(e.target.value) || 0,
                  }))
                }
              />
            </label>
          ...
          <label className="grid gap-1 text-xs">
            ...
            <Input
              ...
              disabled={isLeader}
              onChange={e =>
                setForm(current => ({ ...current, regional: e.target.value }))
              }
              placeholder="Ex.: Regional SP"
            />
          </label>
          <label className="grid gap-1 text-xs">
            Distrito
            <Input
              ...
              disabled={isLeader}
              onChange={e =>
                setForm(current => ({ ...current, district: e.target.value }))
              }
              placeholder="Ex.: Distrito SP Norte"
            />
          </label>
          <label className="grid gap-1 text-xs">
            Polo
            <Input
              ...
              disabled={isLeader}
              onChange={e =>
                setForm(current => ({ ...current, polo: e.target.value }))
              }
              placeholder="Ex.: Polo Vila Medeiros"
            />
          </label>
          <label className="grid gap-1 text-xs">
            Rota
        ...
        {isLeader && (
          <p className="mt-4 rounded-lg bg-amber-50 p-3 text-xs text-amber-900">
            Para corrigir função, regional, distrito, polo ou rota, solicite a
            atualização a um administrador autorizado.
          </p>
        )}
        <label className="mt-5 flex items-center justify-between rounded-lg bg-[#f5f8f2] p-4 text-sm">
          <span>
            <b className="block">Participar do ranking</b>
            <small className="text-emerald-700/65">
              A classificação pública mostra somente nome, polo, distrito e
              pontos.
            </small>
          </span>
          <input
            type="checkbox"
            checked={form.profileVisibleInRanking}
            onChange={e =>
              setForm(current => ({
                ...current,
                profileVisibleInRanking: e.target.checked,
              }))
            }
    ...
    onSuccess: result =>
      toast.success(
        result.pointsAwarded
          ...
          : "RMR registrada. A pontuação deste KPI já foi concedida neste mês."
      ),
    onError: error => toast.error(error.message),
  });
  return (
    <div className="space-y-6">
      <section>
        ...
        <p className="mt-2 text-sm text-emerald-800/65">
          A RMR usa tarefas, propostas e TPV para calcular o KPI global. O
          período rápido ajuda a preparar este registro.
        </p>
      </section>
      <section className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
        <article className="rounded-xl border border-emerald-100 bg-white p-5">
          ...
          >
            <CheckCircle2 size={16} /> Registrar RMR
          </Button>
        </article>
        <article className="rounded-xl bg-[#f6f8f2] p-6">
          ...
          <p className="mt-2 text-sm text-emerald-800/65">
            Acima de 80%: 20 pts; de 100% a 150%: 40 pts; acima de 150%: 100
            pts. A pontuação é concedida uma vez por mês.
          </p>
...
function RankingPanel() {
  ...
  const trophies = trpc.ranking.trophies.useQuery({ monthKey: selectedMonth });
  const tioPatinhas = trpc.ranking.tioPatinhas.useQuery({
    monthKey: selectedMonth,
  });
  const groups = [
    {
      label: "Mais novos clientes",
      items: trophies.data?.newClients ?? [],
      helper: "desempate por TPV",
    },
    {
      label: "Maior TPV",
      items: trophies.data?.tpv ?? [],
      helper: "desempate por clientes",
    },
    {
      label: "Maior KPI global",
      items: trophies.data?.kpi ?? [],
      helper: "desempate por TPV e clientes",
    },
  ];
  return (
    <div className="space-y-6">
      <section>
        ...
        <h2 className="mt-2 text-3xl font-semibold tracking-[-.055em]">
          Reconhecimento mensal baseado em resultado.
        </h2>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        {groups.map(group => (
          ...
          >
            ...
            <p className="mt-1 text-xs text-emerald-700/60">{group.helper}</p>
            <div className="mt-4 space-y-2">
              {group.items.map((agent, index) => (
                <div
                  className="flex items-center gap-3 rounded-lg bg-[#f6f8f2] p-3 text-xs"
                  key={`${group.label}-${agent.displayName}-${index}`}
                >
                  {index === 0 ? (
                    <LaurelTop1 size={28} />
                  ) : (
                    <span className="grid h-7 w-7 place-items-center rounded-full bg-lime-200 font-mono text-emerald-950">
                      {index + 1}
                    </span>
                  )}
                  <div className="min-w-0 flex-1">
                    <b className="block truncate">{agent.displayName}</b>
                    <small className="text-emerald-700/60">
                      {brl.format(agent.actualTpv)} · {agent.actualNewClients}{" "}
                      clientes · KPI {percent.format(agent.globalKpi)}%
                    </small>
                  </div>
                </div>
              ))}
              {!group.items.length && (
                <p className="rounded-lg bg-[#f6f8f2] p-3 text-xs text-emerald-700/65">
                  O troféu aparece após os cards finais do mês serem
                  registrados.
                </p>
              )}
            </div>
      ...
      <section className="rounded-xl border border-emerald-100 bg-white p-5">
        <div className="flex items-center gap-2">
          <TioPatinhasTrophy size={28} />
          <p className="font-mono text-[10px] tracking-[.12em] text-emerald-600">
            TIO PATINHAS
          </p>
        </div>
        <h3 className="mt-1 text-xl font-semibold">
          Variável real, de forma anônima.
        </h3>
        <p className="mt-1 text-sm text-emerald-800/65">
          Acompanhe RV, TPV e a composição por tiers sem expor o nome dos
          agentes.
        </p>
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-xs">
            <thead className="border-b border-emerald-100 font-mono text-[10px] uppercase tracking-[.08em] text-emerald-700/65">
              <tr>
                <th className="pb-3">Arquétipo</th>
                <th className="pb-3">RV realizada</th>
                <th className="pb-3">TPV</th>
                <th className="pb-3">15–30k</th>
                <th className="pb-3">30–50k</th>
                <th className="pb-3">50–100k</th>
                <th className="pb-3">100k+</th>
              </tr>
            </thead>
            <tbody>
              {tioPatinhas.data?.map(item => (
                <tr className="border-b border-emerald-50" key={item.alias}>
                  <td className="py-3 font-semibold">{item.alias}</td>
                  <td className="py-3 font-mono">
                    {brl.format(item.actualVariable)}
                  </td>
                  <td className="py-3 font-mono">
                    {brl.format(item.totalMigratedTpv)}
                  </td>
                  <td className="py-3">{item.clients15To30}</td>
                  <td className="py-3">{item.clients30To50}</td>
                  <td className="py-3">{item.clients50To100}</td>
                  <td className="py-3">{item.clients100Plus}</td>
                </tr>
              ))}
              {!tioPatinhas.isLoading && !tioPatinhas.data?.length && (
                <tr>
                  <td
                    colSpan={7}
                    className="py-7 text-center text-emerald-700/65"
                  >
                    O ranking aparecerá após os cards finais do mês serem
                    registrados.
                  </td>
                </tr>
              )}
      ...
      <section className="overflow-hidden rounded-xl border border-emerald-100 bg-white">
        ...
        {ranking.data?.map((agent, index) => (
          <div
            className="flex items-center gap-4 border-t border-emerald-50 px-5 py-4 text-sm"
            key={`${agent.displayName}-${index}`}
          >
            <span className="grid h-9 w-9 place-items-center rounded-full bg-emerald-50 font-mono text-emerald-700">
              {index + 1}
            </span>
            <div className="min-w-0 flex-1">
              <b>{agent.displayName}</b>
              <small className="block text-emerald-700/60">
                {agent.polo || "Polo não informado"} ·{" "}
                {agent.district || "Distrito não informado"}
              </small>
            </div>
            <b className="font-mono text-emerald-800">{agent.points} pts</b>
          </div>
        ))}
      </section>
    </div>
...
function AgentDashboard({
  ...
  data: {
    ...
    points: number;
    simulations: { finalVariable: number }[];
  };
  onView: (view: View) => void;
}) {
  const latestGoal = data.latestGoal;
  const currentVariable =
    latestGoal?.actualVariable ?? data.simulations[0]?.finalVariable ?? 0;
  const targetVariable =
    latestGoal?.targetVariable ?? data.profile?.targetVariable ?? 0;
  const progress = targetVariable
    ? Math.min((currentVariable / targetVariable) * 100, 100)
    : 0;
  const targetTpv = latestGoal?.targetTpv ?? data.profile?.defaultGoalTpv ?? 0;
  const actualTpv = latestGoal?.actualTpv ?? 0;
  const tpvProgress = targetTpv
    ? Math.min((actualTpv / targetTpv) * 100, 100)
    : 0;
  const pointsLevelProgress = data.points
    ? Math.min(data.points % 100 || 100, 100)
    : 0;
  return (
    <div className="space-y-6">
      ...
      <article className="relative overflow-hidden rounded-xl border border-[#00d47e]/25 bg-[#002b1d] p-6 text-white">
        ...
        <div className="relative">
          <p className="font-mono text-[10px] font-semibold tracking-[.15em] text-[#8fe6bd]">
            NOVA ODISSEIA: FIORATI
          </p>
          <h2 className="mt-2 text-2xl font-semibold tracking-[-.05em]">
            Uma plataforma para transformar profissionais em Outliers.
          </h2>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-emerald-100/75">
            Organize a rota, planeje prioridades, proteja o foco e transforme
            dados em planos de ação. A jornada desenvolve raciocínio rápido,
            adaptabilidade, persuasão e comunicação excepcional com prática
            deliberada.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            {[
              ...
              <span
                className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs text-emerald-50"
                key={item}
              >
                {item}
              </span>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <KpiGauge
          label="RV REALIZADA"
          value={brl.format(currentVariable)}
          helper={
            targetVariable
              ? `${percent.format(progress)}% da RV alvo`
              : "Defina sua RV no perfil"
          }
        ...
        <KpiGauge
          label="META TPV"
          value={brl.format(targetTpv)}
          helper="objetivo mensal"
          progressPercent={100}
          icon={Gauge}
        />
        <KpiGauge
          label="TPV ATUAL"
          value={brl.format(actualTpv)}
          helper={
            targetTpv
              ? `${percent.format(tpvProgress)}% da meta`
              : "registro mais recente"
          }
          progressPercent={tpvProgress}
          icon={BarChart3}
        />
        <KpiGauge
          label="CONQUISTAS"
          value={`${data.points} pts`}
          helper="PSV e KPI mensal"
          progressPercent={pointsLevelProgress}
          icon={Trophy}
        />
      </section>
      <SkillsDashboard
        onOpenSpartacus={() => {
          window.history.replaceState({}, "", "?view=spartacus");
          window.location.reload();
        }}
      />
      <section className="grid gap-4 lg:grid-cols-3">
        <button
          className="rounded-xl border border-emerald-100 bg-white p-5 text-left transition hover:border-lime-300"
          onClick={() => onView("periodo")}
        >
          <Target className="text-emerald-600" />
          <h3 className="mt-3 font-semibold">Período rápido</h3>
          <p className="mt-1 text-sm text-emerald-700/65">
            Defina quantidades por tier e acompanhe plano, realizado e GAP.
          </p>
        </button>
        <button
          className="rounded-xl border border-emerald-100 bg-white p-5 text-left transition hover:border-lime-300"
          onClick={() => onView("nordica")}
        >
          <Medal className="text-emerald-600" />
          <h3 className="mt-3 font-semibold">Estratégia Nórdica</h3>
          <p className="mt-1 text-sm text-emerald-700/65">
            Funis, ativações e microrrotas de visita.
          </p>
        </button>
        <button
          className="rounded-xl bg-[#002b1d] p-5 text-left text-white"
          onClick={() => onView("psv")}
        >
          <ClipboardCheck className="text-lime-200" />
          <h3 className="mt-3 font-semibold">PSV semanal</h3>
          <p className="mt-1 text-sm text-emerald-100/70">
            Transforme a meta em próximas oportunidades.
          </p>
        </button>
      </section>
    </div>
...
export default function Home() {
  ...
  const isLeader =
    role !== "none" && !(user.role === "admin" && view === "psv-ritual");
  ...
  const content = () => {
    ...
    if (view === "nordica" && user.role === "admin") {
      return (
        <NordicStrategyPanel onOpenPeriod={() => navigateToView("periodo")} />
      );
    }
    ...
    if (view === "periodo") {
      return isLeader ? (
        ...
        <QuickPeriodPanel onOpenPsv={() => navigateToView("psv")} />
      );
    }
    ...
    if (view === "card-final") {
      return isLeader ? <LeadershipPanel /> : <MonthlyFinalCardPanel />;
    }
    ...
    if (view === "perfil") {
      return <ProfilePanel profile={data?.profile ?? null} />;
    }
    ...
    return data ? <AgentDashboard data={data} onView={navigateToView} /> : null;
  };
          ...
          <nav className="mt-10 space-y-1">
            {nav.map(([key, Icon, label]) => (
              <button
                key={key}
                type="button"
                onPointerDown={event => handleNavigationPointerDown(event, key)}
        ...
        <main className="min-w-0 flex-1">
          <header className="sticky top-0 z-20 flex items-center justify-between border-b border-emerald-100 bg-[#f4f3ec]/95 px-5 py-4 backdrop-blur lg:px-9">
            ...
            <div className="hidden items-center gap-2 text-xs text-emerald-700/65 lg:flex">
              ...
              {isLeader ? "Espaço de gestão" : "Espaço privado do agente"}
            </div>

            <div className="text-right">
              <b className="block text-sm">
                {data?.profile?.displayName || user.name || "Agente"}
              </b>
        ...
        {nav
          ...
          <button
            ...
            className={`grid place-items-center gap-1 rounded-md px-2 py-1 text-[9px] ${
              view === key ? "text-emerald-700" : "text-emerald-700/50"
            }`}
          >
            <Icon size={17} />
