...
function Demands() {
  ...
  return <section className="space-y-4"><header><p className="font-mono text-[10px] tracking-[.14em] text-[#3ddc84]">DEMANDAS</p><h3 className="mt-2 text-2xl font-semibold">Tudo que precisa acontecer, por data.</h3></header><div className="grid gap-3 rounded-[10px] border border-[#1f3a2e] bg-[#10201a] p-4 md:grid-cols-[1fr_180px_160px_auto]"><Input className="border-[#1f3a2e] bg-[#0b1410] text-[#edefe9]" placeholder="Nova demanda" value={title} onChange={event => setTitle(event.target.value)} /><Input className="border-[#1f3a2e] bg-[#0b1410] text-[#edefe9]" placeholder="Categoria" value={category} onChange={event => setCategory(event.target.value)} /><Input className="border-[#1f3a2e] bg-[#0b1410] text-[#edefe9]" type="date" value={dueDate} onChange={event => setDueDate(event.target.value)} /><Button className="bg-[#3ddc84] text-[#0b1410]" disabled={save.isPending || !title.trim()} onClick={() => save.mutate({ title, category, dueDate })}><Plus size={16} /> Adicionar</Button></div><div className="space-y-2">{list.map(item => <div key={item.id} className={`flex items-center gap-3 rounded-lg border border-[#1f3a2e] bg-[#10201a] p-3 ${item.completed ? "opacity-55" : ""}`}><input type="checkbox" checked={item.completed} onChange={event => save.mutate({ id: item.id, title: item.title, category: item.category, dueDate: item.dueDate, completed: event.target.checked })} /><div className="min-w-0 flex-1"><b className={item.completed ? "line-through" : ""}>{item.title}</b><span className="ml-2 text-[10px] text-[#9fb0a6]">{item.category} · {new Date(`${item.dueDate}T12:00:00`).toLocaleDateString("pt-BR")}</span></div><Button variant="ghost" size="sm" className="text-[#e25c5c]" onClick={() => remove.mutate({ id: item.id })}><Trash2 size={15} /></Button></div>)}{!list.length && <p className="rounded-lg border border-dashed border-[#1f3a2e] p-5 text-sm text-[#9fb0a6]">Nenhuma demanda cadastrada.</p>}</div></section>;
}
...
function Development() {
  ...
  return <section className="space-y-5"><header><p className="font-mono text-[10px] tracking-[.14em] text-[#3ddc84]">MEU DESENVOLVIMENTO</p><h3 className="mt-2 text-2xl font-semibold">Pessoal e profissional no mesmo ritmo.</h3><p className="mt-1 text-sm text-[#9fb0a6]">O PDI SPARTACUS continua privado e agora vive dentro da sua rotina de gestão.</p></header><div className="grid gap-3 sm:grid-cols-3"><article className="rounded-[10px] border border-[#1f3a2e] bg-[#10201a] p-4"><BarChart3 className="text-[#3ddc84]" size={18} /><b className="mt-3 block text-2xl">{kpi}%</b><span className="text-xs text-[#9fb0a6]">KPI de TPV atual</span></article><article className="rounded-[10px] border border-[#1f3a2e] bg-[#10201a] p-4"><Target className="text-[#3ddc84]" size={18} /><b className="mt-3 block text-2xl">{plans.length}</b><span className="text-xs text-[#9fb0a6]">planos SPARTACUS salvos</span></article><article className="rounded-[10px] border border-[#1f3a2e] bg-[#10201a] p-4"><UserRound className="text-[#3ddc84]" size={18} /><b className="mt-3 block text-2xl">{dashboard.data?.points ?? 0}</b><span className="text-xs text-[#9fb0a6]">pontos de evolução</span></article></div><SpartacusPanel /></section>;
}
...
export default function UnifiedPsvPanel({ profile, currentVariable, latestDetailsJson }: { profile: { targetVariable: number } | null; currentVariable: number; latestDetailsJson?: string | null }) {
  ...
  return <div className="min-h-full rounded-xl bg-[#0b1410] p-4 text-[#edefe9] md:p-7"><header className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b border-[#1f3a2e] pb-5"><div><p className="font-mono text-[10px] tracking-[.16em] text-[#3ddc84]">PLANO SEMANAL</p><h2 className="mt-2 text-3xl font-semibold">Quanto preciso fazer por dia.</h2><p className="mt-2 text-sm text-[#9fb0a6]">Gestão comercial, demandas, acompanhamento e desenvolvimento em um só lugar.</p></div><div className="flex items-center gap-2 text-xs text-[#9fb0a6]"><ClipboardList size={17} className="text-[#3ddc84]" /> rotina viva</div></header><nav className="mb-7 flex max-w-full gap-1 overflow-x-auto rounded-[14px] border border-[#1f3a2e] bg-[#10201a] p-1">{tabs.map(([key, label, Icon]) => <button type="button" key={key} onClick={() => setSection(key)} className={`flex shrink-0 items-center gap-2 rounded-[10px] px-3 py-2 text-xs font-semibold transition ${section === key ? "bg-[#3ddc84] text-[#0b1410]" : "text-[#9fb0a6] hover:text-[#edefe9]"}`}><Icon size={15} />{label}</button>)}</nav>{section === "comercial" && <div className="space-y-7"><MatrixPsvPanel profile={profile} currentVariable={currentVariable} latestDetailsJson={latestDetailsJson} /><PsvRitualPanel /></div>}{section === "demandas" && <Demands />}{section === "diario" && <DailyFollowUp />}{section === "desenvolvimento" && <Development />}</div>;
}
