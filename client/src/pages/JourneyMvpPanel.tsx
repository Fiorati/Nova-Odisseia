...
export default function JourneyMvpPanel({ userId }: { userId: number }) {
  ...
  const [energy, setEnergy] = useState(savedToday?.energy ?? 3); const [reflection, setReflection] = useState(savedToday?.reflection ?? ""); const [nextAction, setNextAction] = useState(savedToday?.nextAction ?? "");
  ...
  return <div className="space-y-6">
    ...
    <article className="rounded-xl border border-emerald-100 bg-white p-5"><p className="font-mono text-[10px] tracking-[.12em] text-emerald-600">CHECK-IN DE HOJE</p><h3 className="mt-1 text-xl font-semibold">Pausa. Presença. Próximo passo.</h3><label className="mt-4 block text-xs">Energia: <b>{energy}/5</b><input className="mt-2 w-full accent-emerald-700" type="range" min="1" max="5" value={energy} onChange={e=>setEnergy(Number(e.target.value))}/></label><label className="mt-4 block text-xs">Como foi seu dia, em uma frase?<textarea value={reflection} onChange={e=>setReflection(e.target.value)} rows={2} className="mt-2 w-full rounded-md border border-emerald-100 p-3 text-sm" placeholder="Ex.: Fiz 3 visitas e senti mais confiança na abordagem."/></label><label className="mt-3 block text-xs">Qual é a próximo passo concreto (e quando)?<Input value={nextAction} onChange={e=>setNextAction(e.target.value)} className="mt-2" placeholder="Ex.: Amanhã às 9h ligar para 5 clientes da carteira."/></label><Button onClick={saveCheckin} className="mt-4 w-full bg-[#0e3426]"><Sparkles size={16}/> Salvar check-in</Button></article></section>
    <section className="rounded-xl border border-emerald-100 bg-white p-5"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] tracking-[.12em] text-emerald-600">DIÁRIO DE BORDO</p><h3 className="mt-1 text-xl font-semibold">O que você registrou.</h3></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800">{state.checkins.length} registro(s)</span></div><div className="mt-5 space-y-3">{state.checkins.length ? state.checkins.slice(0,7).map(item => <article key={item.date} className="rounded-lg border border-emerald-100 bg-[#f7f8f3] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><b className="text-sm">{new Date(`${item.date}T12:00:00`).toLocaleDateString("pt-BR", {day:"2-digit", month:"long", year:"numeric"})}</b><span className="font-mono text-xs text-emerald-700">ENERGIA {item.energy}/5</span></div><div className="mt-3 grid gap-3 md:grid-cols-2"><div><small className="font-mono text-[9px] tracking-[.1em] text-emerald-600">O QUE PERCEBI</small><p className="mt-1 text-sm leading-6 text-emerald-950">{item.reflection}</p></div><div><small className="font-mono text-[9px] tracking-[.1em] text-emerald-600">PRÓXIMA AÇÃO</small><p className="mt-1 text-sm leading-6 text-emerald-950">{item.nextAction}</p></div></div></article>) : <p className="rounded-lg bg-[#f7f8f3] p-4 text-sm text-emerald-800/60">Seu primeiro check-in aparecerá aqui, completo.</p>}</div></section>
