...
export default function JourneyMvpPanel({ userId }: { userId: number }) {
  ...
  const saveCheckin = async () => { if (!reflection.trim() || !nextAction.trim()) return toast.error("Registre o aprendizado e a próxima ação."); const today = new Date().toISOString().slice(0,10); const prior = state.checkins.some(item => item.date === today); const xp = state.xp + (prior ? 0 : 10); const nextState = { ...state, xp, stage: stageForXp(xp), streak: prior ? state.streak : state.streak + 1, checkins: [{ date: today, energy, reflection: reflection.trim(), nextAction: nextAction.trim() }, ...state.checkins.filter(item => item.date !== today)].slice(0,30) }; setState(nextState); await saveJourney("Check-in", nextState); };
