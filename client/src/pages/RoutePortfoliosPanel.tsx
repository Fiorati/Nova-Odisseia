...
async function parseFile(file: File): Promise<ParsedPortfolioRow[]> {
...
function PortfolioTab({
  ...
  const [pendingImport, setPendingImport] = useState<{ scope: ImportScope; input: PortfolioImportPayload } | null>(null);
  ...
  const isLeader = leadership !== "none";
  ...
  const stages = useMemo(
    () =>
      Array.from(
        new Set(entries.map(entry => entry.stage).filter(Boolean))
      ).sort(),
    [entries]
  );
  ...
  const filteredEntries = useMemo(
    ...
    [cityFilter, entries, stageFilter]
  );
    ...
    toast.success(
      importScope === "route"
        ...
        : data.waitingRoutes.length
          ? `${data.rowCount} linhas importadas; ${data.waitingRoutes.length} rota(s) aguardam titularidade.${duplicates}`
          : `${data.rowCount} linhas distribuídas com sucesso.${duplicates}`
    );
  };
  ...
  const upload = async (file: File | undefined, importScope: ImportScope) => {
    ...
    try {
      ...
      if (!rows.length)
        throw new Error(
          "Não encontrei uma coluna de cliente preenchida no arquivo."
        );
      if (importScope !== "route" && rows.some(row => !row.route))
        throw new Error(
          "A importação por polo ou distrito exige Cliente e Rota preenchidos em todas as linhas."
        );
      const input = {
        portfolioType: type,
        fileName: file.name,
      ...
      setPendingImport({ scope: importScope, input });
      toast.success(`${rows.length} clientes carregados. Clique em Salvar importação para persistir.`);
    } catch (error) {
      toast.error(
  ...
  return (
    <section className="rounded-xl border border-emerald-100 bg-white p-5">
      {pendingImport && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-lime-300 bg-lime-50 p-4 text-sm text-emerald-950">
          <span><b>{pendingImport.input.rows.length} clientes prontos para salvar</b> · arquivo {pendingImport.input.fileName}</span>
          <div className="flex gap-2"><Button className="bg-[#002b1d]" disabled={pending} onClick={savePendingImport}>Salvar importação</Button><Button variant="outline" onClick={() => setPendingImport(null)}>Descartar</Button></div>
        </div>
      )}
      ...
      {portfolio.data?.importInfo && (
        <div className="mt-3 rounded-lg border border-emerald-100 bg-emerald-50/50 px-4 py-3 text-xs text-emerald-900">
          ...
          · <b>Domínio/origem:</b> {portfolio.data.importInfo.sourceOrigin} ·{" "}
          <b>Importado em:</b>{" "}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {portfolio.data!.referenceMonths.slice(0, 6).map(month => (
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs ${visibleMonth === month ? "bg-[#002b1d] text-white" : "bg-emerald-50 text-emerald-700"}`}
            >
              {month}
            </button>
          ))}
              ...
              {!focusListIntelligent && (
                <>
                  <input
                    ...
                    onChange={event => {
                      void upload(event.target.files?.[0], "route");
          ...
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
            <div>
              ...
              <span className="text-xs text-emerald-800/70">
                A coluna Rota é obrigatória. Linhas sem titularidade permanecem
                aguardando o vínculo.
              </span>
            </div>
            <div className="flex flex-wrap items-end gap-2">
              ...
              <input
                ...
                accept=".csv,.xlsx,.xls,.pdf,text/csv,application/pdf"
                onChange={event => {
                  void upload(event.target.files?.[0], scope);
                  event.currentTarget.value = "";
                }}
              />
              <Button
            ...
            <div className="mt-4 grid gap-3 rounded-lg border border-emerald-100 bg-white p-4 md:grid-cols-[1fr_auto]">
              <div>
                ...
                <p className="mt-2 text-xs text-emerald-800/65">
                  A primeira aba será importada; a planilha deve estar liberada
                  para visualização por link.
                </p>
              </div>
              <Button
                className="self-end bg-[#002b1d]"
            ...
            {priorityEntries.map((entry, index) => (
              <div key={entry.id} className="rounded-lg bg-white p-3 shadow-sm">
                ...
                <p className="mt-1 text-xs text-emerald-800/65">
                  ...
                  {entry.stage || "Etapa não informada"}
                </p>
      ...
      <div className="mt-5 flex flex-wrap items-end gap-3 rounded-lg bg-[#f6f8f2] p-3">
        ...
        <label className="text-xs text-emerald-800/70">
          Etapa
          <select
            className="ml-2 rounded-md border border-emerald-100 bg-white px-2 py-1.5"
            value={stageFilter}
      ...
      <div className="mt-5 overflow-x-auto">
        <table className="w-full min-w-[1380px] text-left text-xs">
          ...
          <tbody>
            {portfolio.isLoading ? (
              <tr>
                <td colSpan={10} className="py-6 text-emerald-700/65">
              ...
              filteredEntries.map(entry => (
                <tr className="border-b border-emerald-50" key={entry.id}>
                  ...
                  <td className="py-3">{entry.stage || "—"}</td>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-emerald-800/65">
          Anexe o print ou arquivo exportado da sua operação. A rota transforma
          a origem em uma lista prática de clientes, etapas e próximas ações.
        </p>
      </div>
      <PortfolioTab type={active} focusListIntelligent={focusListIntelligent} />
      {<RouteAssignmentsPanel enabled={isLeader} />}
    </div>
  );
}
